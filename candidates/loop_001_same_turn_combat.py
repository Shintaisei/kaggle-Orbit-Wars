# Loop 001: same_turn_combat
# Based on candidate_02_less_defense_absorption. This variant keeps the current
# defense-budget hypothesis and fixes ETA-grouped combat projections.
...
def effective_garrison_at_arrival(target, travel_turns, world):
    ...
    for eta, fleet_owner, fleet_ships in relevant:
        by_turn[int(eta)].append((int(fleet_owner), int(fleet_ships)))

    for eta in sorted(by_turn):
        if owner != -1:
            ships += prod * (eta - last_t)
        owner, ships = _resolve_combat(owner, ships, by_turn[eta])
        last_t = eta
    
    if owner != -1:
        ships += prod * (travel_turns - last_t)
    return owner, int(max(0, ships))
...
def compute_planet_reserve(planet, arrivals, player):
    ...
    for eta, owner, ships in arrivals:
        if ships <= 0:
            continue
        if owner == player:
            by_turn[int(eta)].append((int(owner), int(ships)))
        elif owner == -1:
            continue
        else:
    ...
    def simulate(initial_ships):
        owner = int(player)
        bal = float(initial_ships)
        last_t = 0
        min_bal = bal
        deadline = None
        for turn in sorted(by_turn):
            ...
            owner, bal = _resolve_combat(owner, bal, by_turn[turn])
            if owner != player:
                return owner, bal, False, turn
            if bal < min_bal:
                min_bal = bal
            if bal < ABSORB_PROJECTION_MARGIN and deadline is None:
                deadline = turn
            last_t = turn
        return owner, bal, min_bal >= ABSORB_PROJECTION_MARGIN, deadline

    full_owner, full_bal, full_holds, full_deadline = simulate(ships_now)
    if not full_holds:
        extra = 0
        total_arrival_ships = sum(int(s) for events in by_turn.values() for _owner, s in events)
        max_extra = max(ships_now + 1, total_arrival_ships + ABSORB_PROJECTION_MARGIN + 20)
        while extra <= max_extra:
            _owner, _bal, holds, deadline = simulate(ships_now + extra)
            if holds:
                return ships_now, False, int(extra), deadline
            extra += 1
        deficit = max(1, int(ABSORB_PROJECTION_MARGIN - full_bal))
        return ships_now, False, deficit, full_deadline

    lo, hi = 0, ships_now
    while lo < hi:
        mid = (lo + hi) // 2
        _owner, _bal, holds, _deadline = simulate(mid)
        if holds:
            hi = mid
        else:
            lo = mid + 1
    return int(lo), True, 0, None
