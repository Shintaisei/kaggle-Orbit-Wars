# Loop 002: fleet_sun_filter
# Based on champion_current. This variant prevents sun-crashing existing fleets
# from entering the inferred arrival ledger.
...
def segment_hits_sun_strict(x1, y1, x2, y2):
    return point_to_segment_distance(CENTER_X, CENTER_Y, x1, y1, x2, y2) < SUN_R
...
def fleet_target_planet(fleet, planets, initial_by_id=None, ang_vel=0.0):
    ...
    for p in planets:
        ...
        hit_x = fleet.x + dx_dir * hit_d
        hit_y = fleet.y + dy_dir * hit_d
        if segment_hits_sun_strict(fleet.x, fleet.y, hit_x, hit_y):
            continue
        t = hit_d / speed
    ...
    if initial_by_id is not None:
        ...
        for t in range(1, max_t + 1):
            ...
            for p in planets:
                ...
                dsq = (fx - px) ** 2 + (fy - py) ** 2
                if dsq < rr:
                    if segment_hits_sun_strict(fleet.x, fleet.y, fx, fy):
                        continue
