...
def _trace_commit_guard(world, src_id, target_id, angle, turns, ships):
    trace_path = os.environ.get("ORBIT_GUARD_TRACE")
    if not trace_path:
        return
    try:
        ...
        sun = segment_hits_sun(lx, ly, ex, ey)
        ...
        first_id = None if first_target is None else int(first_target.id)
        first_owner = None if first_target is None else int(first_target.owner)
        first_static = None if first_target is None else int(is_static_planet(first_target))
        reason = []
        if sun:
            reason.append("predicted_path_hits_sun")
        if first_id is not None and first_id != int(target_id):
            reason.append("first_target_mismatch")
        if not reason:
            return
        with open(trace_path, "a") as fh:
            fh.write(
                " ".join([
                    ...
                    f"ships={int(ships)}",
                    f"turns={int(turns)}",
                    f"target_owner={int(target.owner)}",
                    f"target_static={int(is_static_planet(target))}",
                    f"first_id={first_id}",
                    f"first_owner={first_owner}",
                    f"first_static={first_static}",
                    f"first_turns={first_turns}",
        ...
        try:
            with open(trace_path, "a") as fh:
                fh.write(f"step={getattr(world, 'step', -1)} guard_trace_error={type(exc).__name__}\n")
        except Exception:
            pass
...
def _commit_fleet(world, moves, spent, target_locked,
    ...
    _trace_commit_guard(world, src_id, target_id, angle, turns, ships)
