...
def _solo_capture_hits_own_planet_first(world, src, tgt, angle, turns, ships):
    ...
    first_target, first_turns = fleet_target_planet(
        Fleet(-1, world.player, lx, ly, float(angle), int(src.id), int(ships)),
        world.planets,
        world.initial_by_id,
        world.ang_vel,
    )
    if first_target is None:
        return False
    if int(first_target.id) == int(tgt.id):
        return False
    if int(first_target.owner) != int(world.player):
        return False
    return first_turns is not None and int(first_turns) < int(turns)
...
def plan_solo_capture(world, src, tgt, max_avail, max_travel):
    ...
    if FWD_SIM_FILTER_ENABLED and not world.is_2p and tgt.owner == -1:
        ...
        if end_owner != world.player and end_owner != -1 and end_ships > 5:
            return None
    if _solo_capture_hits_own_planet_first(world, src, tgt, angle, turns, ships):
        return None
