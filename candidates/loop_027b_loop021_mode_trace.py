# Adapted from public Kaggle Notebook:
# vickimar/orbit-wars-heuristic-lb-1110 ("orbit-wars-heuristic [LB 1110+]").
# ---
# jupyter:
#   jupytext:
#     text_representation:
...
def _trace_relation(owner, player):
    if owner is None:
        return "none"
    if int(owner) == -1:
        return "neutral"
...
def _trace_commit_mode(world, spent, src_id, target_id, angle, turns, ships, mode_label, commit_allowed=True):
    trace_path = os.environ.get("ORBIT_MODE_TRACE")
    if not trace_path:
        return
    try:
        src = world.planet_by_id.get(int(src_id))
        target = world.planet_by_id.get(int(target_id))
        src_ships = int(src.ships) if src is not None else None
        spent_before = int(spent[int(src_id)])
        ships_i = int(ships)
        overbudget = src_ships is not None and spent_before + ships_i > src_ships
        first_target = None
        first_turns = None
        predicted_sun = False
        if src is not None and target is not None:
            lx, ly = launch_point(float(src.x), float(src.y), float(src.radius), float(angle))
            target_pos = predict_target_position(target, world, int(turns))
            if target_pos is not None:
                tx, ty = target_pos
                hit_d = max(0.0, dist(lx, ly, float(tx), float(ty)) - float(target.radius))
                ex = lx + math.cos(float(angle)) * hit_d
                ey = ly + math.sin(float(angle)) * hit_d
                predicted_sun = segment_hits_sun(lx, ly, ex, ey)
            first_target, first_turns = fleet_target_planet(
                Fleet(-1, world.player, lx, ly, float(angle), int(src_id), ships_i),
                world.planets,
                world.initial_by_id,
                world.ang_vel,
            )
        target_owner = int(target.owner) if target is not None else None
        first_owner = int(first_target.owner) if first_target is not None else None
        first_id = int(first_target.id) if first_target is not None else None
        first_mismatch = first_id is not None and first_id != int(target_id)
        first_before_target = first_turns is not None and int(first_turns) < int(turns)
        pending = [c for c in _pending_commitments if int(c.get("target_id", -999)) == int(target_id)]
        row = {
            "step": int(world.step),
            "player": int(world.player),
            "is_2p": bool(world.is_2p),
            "world_mode": getattr(world, "mode", None),
            ...
            "spent_plus_ships": spent_before + ships_i,
            "overbudget": bool(overbudget),
            "commit_allowed": bool(commit_allowed),
            "target_owner": target_owner,
            "target_relation": _trace_relation(target_owner, world.player),
            "target_static": None if target is None else bool(is_static_planet(target)),
            "target_ships": None if target is None else int(target.ships),
            "target_prod": None if target is None else int(target.production),
            "target_is_comet": bool(target is not None and target.id in world.comet_ids),
            "first_id": first_id,
            "first_owner": first_owner,
            "first_relation": _trace_relation(first_owner, world.player),
            "first_static": None if first_target is None else bool(is_static_planet(first_target)),
            "first_turns": None if first_turns is None else int(first_turns),
            "first_is_comet": bool(first_target is not None and first_target.id in world.comet_ids),
            "first_target_mismatch": bool(first_mismatch),
            "predicted_path_hits_sun": bool(predicted_sun),
            "first_before_target": bool(first_before_target),
            "first_eta_delta": None if first_turns is None else int(first_turns) - int(turns),
            "pending_count": len(pending),
            "pending_ships": sum(int(c.get("ships", 0)) for c in pending),
        }
        suspicious = (
            row["first_target_mismatch"]
            or row["predicted_path_hits_sun"]
            or row["overbudget"]
            or not row["commit_allowed"]
            or row["mode_label"] == "unknown"
        )
        if not suspicious and os.environ.get("ORBIT_MODE_TRACE_ALL") != "1":
            return
        ...
        try:
            with open(trace_path, "a") as fh:
                fh.write(json.dumps({"trace_error": type(exc).__name__, "step": getattr(world, "step", None), "mode_label": str(mode_label)}) + "\n")
        except Exception:
            pass


def _can_commit_fleet(world, spent, src_id, ships):
    src = world.planet_by_id.get(int(src_id))
    if src is None:
        return False
    ships = int(ships)
    if ships <= 0:
        return False
    return int(spent[int(src_id)]) + ships <= int(src.ships)


def _can_commit_many(world, spent, landings):
    needed_by_src = defaultdict(int)
    for src_id, ships in landings:
        ships = int(ships)
        if ships <= 0:
            return False
        needed_by_src[int(src_id)] += ships
    for src_id, ships in needed_by_src.items():
        src = world.planet_by_id.get(int(src_id))
        if src is None:
            return False
        if int(spent[int(src_id)]) + ships > int(src.ships):
            return False
    return True


def _commit_fleet(world, moves, spent, target_locked,
                  src_id, target_id, angle, turns, ships, *, mode_label="unknown"):
    ...
    commit_allowed = _can_commit_fleet(world, spent, src_id, ships)
    _trace_commit_mode(world, spent, src_id, target_id, angle, turns, ships, mode_label, commit_allowed=commit_allowed)
    ...
    for victim_id, (deficit, deadline, victim) in rescue_needs.items():
        ...
        if PREEMPTIVE_DOOM_EVAC_ENABLED and (not PREEMPTIVE_DOOM_EVAC_2P_ONLY or world.is_2p):
            ...
            if threat_metric > garrison_at_deadline * PREEMPTIVE_EVAC_DOOM_RATIO:
                if _try_doom_evac(
                    world, victim, available, spent, target_locked, moves, mode_log,
                    mode_prefix="doom-evac-preemptive",
                ):
                    continue
            ...
            for _t, src_id, src, _angle_est, avail in solo:
                ...
                if FWD_SIM_DEFENSE_CHECK and not world.is_2p:
                    ...
                    if end_owner != world.player:
                        ...
                        continue
                if not _commit_fleet(world, moves, spent, target_locked,
                                     src_id, victim_id, angle, turns, int(send), mode_label="defense-solo"):
                    last_fail = "defense-overbudget"
                    continue
        ...
        if not COALITION_ENABLED:
            ...
            if _try_doom_evac(
                world, victim, available, spent, target_locked, moves, mode_log,
                mode_prefix="doom-evac-fallback",
            ):
                continue
        ...
        if coalition is None:
            ...
            mode_log[victim_id] = "doomed"
            continue
        if not _can_commit_many(world, spent, [(src_id, ships) for src_id, _src, _angle, ships, _turns in coalition]):
            if _try_doom_evac(
                world, victim, available, spent, target_locked, moves, mode_log,
                mode_prefix="doom-evac-fallback",
            ):
                continue
            mode_log[victim_id] = "defense-coalition-overbudget"
            continue
        for src_id, src, angle, ships, turns in coalition:
            _commit_fleet(world, moves, spent, target_locked,
                          src_id, victim_id, angle, turns, int(ships), mode_label="defense-coalition")
            mode_log[src_id] = "defense-coalition"
        mode_log[victim_id] = "defended-by-coalition"


def _try_doom_evac(world, victim, available, spent, target_locked, moves, mode_log, mode_prefix="doom-evac"):
    ...
    if not DOOM_EVAC_ENABLED:
        return False
    garrison = int(victim.ships) - spent[victim.id]
    ...
    if not _commit_fleet(
        world, moves, spent, target_locked,
        victim.id, dst.id, angle, turns, int(garrison),
        mode_label=f"{mode_prefix}-attack",
    ):
        return False
...
def handle_comet_evac(world, available, spent, target_locked, moves, mode_log):
    ...
    for src in world.my_planets:
        ...
        if turns >= rem:
            ...
            pass
        if _commit_fleet(world, moves, spent, target_locked,
                         src.id, best.id, angle, turns, int(avail), mode_label="comet-evac"):
    ...
    for src in sources:
        ...
        for _cost, _eff, n in candidates:
            ...
            if not _neutral_tempo_ok(world, n, int(ships), turns):
                continue
            if _commit_fleet(world, moves, spent, target_locked,
                             src.id, n.id, angle, turns, int(ships), mode_label="cheap-pickup"):
    ...
    for act in actions[:SEARCH_MAX_ACTIONS_TO_PICK * 2]:
        ...
        if tgt is not None and tgt.owner == -1:
            ...
            if not _neutral_tempo_ok(world, tgt, ships_act, turns_act):
                continue
        if not _commit_fleet(world, moves, spent, target_locked,
                             src_id, tgt_id, act["angle"], act["arrival_turn"], act["ships"], mode_label="search-expand"):
            continue
    ...
    for src in sources:
        ...
        for tgt, _approx_dist in candidates:
            ...
            if (
                ...
                continue
            if not _commit_fleet(world, moves, spent, target_locked,
                                 src.id, tgt.id, angle, turns, int(ships), mode_label="expand-solo"):
                continue
...
def _try_coalition_expand(world, src, tgt, max_travel, available, spent,
    ...
    for est_turns, p, p_avail in partners:
        ...
        if not _can_commit_many(world, spent, [(src.id, int(s_src)), (p.id, int(s_p))]):
            continue
        ok_src = _commit_fleet(world, moves, spent, target_locked,
                               src.id, tgt.id, a_src, t_src, int(s_src), mode_label="expand-coalition")
        ok_p = _commit_fleet(world, moves, spent, target_locked,
                             p.id, tgt.id, a_p, t_p, int(s_p), mode_label="expand-coalition")
        if not (ok_src and ok_p):
            continue
...
def _brain_reserve_lead(world, available, spent, mode_log):
    ...
    if BRAIN_LEAD_RESERVE_REQUIRE_TARGET:
        ...
        for tgt in world.enemy_planets:
            if int(tgt.ships) > MEGA_HAMMER_TARGET_GARRISON_MAX:
                continue
...
def handle_accumulator(world, available, spent, target_locked, moves, mode_log):
    ...
    for turns, surplus, src, angle in feeders:
        if fed_count >= ACCUMULATOR_MAX_FEEDS_PER_TURN:
            break
        if _commit_fleet(world, moves, spent, target_locked,
                         src.id, lead.id, angle, turns, int(surplus), mode_label="accumulator-feeder"):
    ...
    for src in sources:
        ...
        if MEGA_HAMMER_MELIS_VERIFY and turns > 0:
            ...
            if end_owner != world.player:
                continue
        if not _commit_fleet(world, moves, spent, target_locked,
                             src.id, tgt.id, angle, turns, int(avail), mode_label="mega-hammer"):
            continue
...
def handle_hammer(world, available, spent, target_locked, moves, mode_log):
    ...
    for src_id, launch in list(plan["launches"].items()):
        if launch.get("fired") or launch["fire_turn_abs"] > world.step:
            continue
        src = world.planet_by_id.get(src_id)
        ships = int(launch["ships"])
        if src is None or src.owner != world.player or ships < HAMMER_MIN_PER_CONTRIBUTOR:
            _hammer_plan = None
            return
        if available[src_id] - spent[src_id] < ships:
            _hammer_plan = None
            return
        target = world.planet_by_id.get(plan["target_id"])
        if target is None:
        ...
        if aim is None:
            ...
            return
        angle, turns = aim
        due_launches.append((src_id, launch, angle, turns, ships))

    if due_launches and not _can_commit_many(world, spent, [(src_id, ships) for src_id, _launch, _angle, _turns, ships in due_launches]):
        _hammer_plan = None
        return

    for src_id, launch, angle, turns, ships in due_launches:
        if not _commit_fleet(world, moves, spent, target_locked,
                             src_id, plan["target_id"], angle, turns, int(ships), mode_label="hammer"):
            _hammer_plan = None
            return
...
def _build_hammer_plan(world, available, spent):
    ...
    for tgt in targets:
        ...
        reaimed = []
        for _turns, src, ships, _angle in chosen:
            aim = aim_at_target(src, tgt, int(ships), world.initial_by_id, world.ang_vel, world=world)
            if aim is None:
                reaimed = []
                break
            angle, turns = aim
            if turns > HAMMER_MAX_TRAVEL:
                reaimed = []
                break
            reaimed.append((int(turns), src, int(ships), float(angle)))
        if not reaimed:
            continue
        chosen = reaimed
        target_arrival = max(c[0] for c in chosen)
        d_owner, d_ships = predict_defender_at_arrival(world, tgt, target_arrival)
        if d_owner == world.player:
            continue
        required = int(math.ceil(d_ships * overkill)) + 1
        if sum(c[2] for c in chosen) < required:
            continue
...
def handle_multiprong(world, available, spent, target_locked, moves, mode_log):
    ...
    for reinforcer, in_flight in candidates:
        ...
        if not _can_commit_many(world, spent, [(src_id, int(ships)) for src_id, _src, _angle, ships, _turns in prong_landings]):
            continue
        committed_prongs = []
        for src_id, src, angle, ships, turns in prong_landings:
            if not _commit_fleet(
                world, moves, spent, target_locked,
                src_id, reinforcer.id, angle, turns, int(ships), mode_label="multiprong",
            ):
                committed_prongs = []
                break
            committed_prongs.append(src_id)
        if not committed_prongs:
            continue
        for src_id in committed_prongs:
