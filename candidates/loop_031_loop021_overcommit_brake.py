...
OVERCOMMIT_BRAKE_2P_ENABLED = True
OVERCOMMIT_BRAKE_TURN_MIN_2P = 80
OVERCOMMIT_BRAKE_FLEET_RATIO_2P = 0.75
OVERCOMMIT_BRAKE_PROD_MARGIN_2P = 5
OVERCOMMIT_BRAKE_RAW_MARGIN_2P = 150
...
def _commit_fleet(world, moves, spent, target_locked,
    ...
    if not _can_commit_fleet(world, spent, src_id, ships):
        return False
...
def _overcommit_pressure_brake_2p(world):
    if not OVERCOMMIT_BRAKE_2P_ENABLED:
        return False, {}
    if not world.is_2p or world.is_late or world.step < OVERCOMMIT_BRAKE_TURN_MIN_2P:
        return False, {}
    my_total_raw = int(world.owner_strength.get(world.player, 0))
    if my_total_raw <= 0:
        return False, {}
    my_fleet_ships = sum(int(f.ships) for f in world.fleets if f.owner == world.player)
    fleet_ratio = float(my_fleet_ships) / float(max(1, my_total_raw))
    enemy_owners = [owner for owner in world.owner_strength.keys() if owner not in (-1, world.player)]
    if not enemy_owners:
        return False, {}
    max_enemy_raw = max(int(world.owner_strength.get(owner, 0)) for owner in enemy_owners)
    max_enemy_prod = max(int(world.owner_production.get(owner, 0)) for owner in enemy_owners)
    my_prod = int(world.owner_production.get(world.player, 0))
    raw_diff = my_total_raw - max_enemy_raw
    prod_diff = my_prod - max_enemy_prod
    info = {
        "fleet_ratio": fleet_ratio,
        "my_fleet_ships": my_fleet_ships,
        "my_total_raw": my_total_raw,
        "max_enemy_raw": max_enemy_raw,
        "raw_diff": raw_diff,
        "my_prod": my_prod,
        "max_enemy_prod": max_enemy_prod,
        "prod_diff": prod_diff,
    }
    brake = (
        fleet_ratio >= OVERCOMMIT_BRAKE_FLEET_RATIO_2P
        and my_prod <= max_enemy_prod + OVERCOMMIT_BRAKE_PROD_MARGIN_2P
        and raw_diff <= OVERCOMMIT_BRAKE_RAW_MARGIN_2P
    )
    return brake, info


def _trace_overcommit_brake(world, info):
    trace_path = os.environ.get("ORBIT_BRAKE_TRACE")
    if not trace_path:
        return
    try:
        with open(trace_path, "a") as fh:
            fh.write(
                " ".join([
                    ...
                    f"player={int(world.player)}",
                    f"fleet_ratio={info.get('fleet_ratio', 0.0):.4f}",
                    f"my_fleet={int(info.get('my_fleet_ships', 0))}",
                    f"my_raw={int(info.get('my_total_raw', 0))}",
                    f"enemy_raw={int(info.get('max_enemy_raw', 0))}",
                    f"raw_diff={int(info.get('raw_diff', 0))}",
                    f"my_prod={int(info.get('my_prod', 0))}",
                    f"enemy_prod={int(info.get('max_enemy_prod', 0))}",
                    f"prod_diff={int(info.get('prod_diff', 0))}",
                    "brake=1",
                ]) + "\n"
            )
    except Exception:
...
def plan_moves(world, deadline=None):
    global _planet_idle_counts, _promoted_stockpiles, _pending_commitments, _hammer_plan
    ...
    overcommit_brake, brake_info = _overcommit_pressure_brake_2p(world)
    if overcommit_brake:
        _trace_overcommit_brake(world, brake_info)
    ...
    if not _over_budget() and not overcommit_brake:
    ...
    if not _over_budget() and (not overcommit_brake or _hammer_plan is not None):
