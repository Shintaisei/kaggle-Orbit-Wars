...
def handle_defense(world, rescue_needs, available, spent, target_locked,
    ...
    for victim_id, (deficit, deadline, victim) in rescue_needs.items():
        ...
        if PREEMPTIVE_DOOM_EVAC_ENABLED and (not PREEMPTIVE_DOOM_EVAC_2P_ONLY or world.is_2p):
            ...
            enemy_arrivals_for_threat = [
                (eta, owner, ships)
                for eta, owner, ships in enemy_arrivals
                if int(eta) <= int(window)
            ]
            if world.is_2p:
                threat_metric = sum(ships for _eta, _owner, ships in enemy_arrivals_for_threat)
            elif not PREEMPTIVE_EVAC_USE_LARGEST_SINGLE_ENEMY_4P:
