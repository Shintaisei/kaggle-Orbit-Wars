...
def handle_defense(world, rescue_needs, available, spent, target_locked,
    ...
    top_owned_production = sorted(
        (int(p.production) for p in world.my_planets),
        reverse=True,
    )[:2]
    ...
    for victim_id, (deficit, deadline, victim) in rescue_needs.items():
        ...
        if PREEMPTIVE_DOOM_EVAC_ENABLED and (not PREEMPTIVE_DOOM_EVAC_2P_ONLY or world.is_2p):
            ...
            victim_prod = int(victim.production)
            is_core_victim = (
                victim_prod >= 3
                or (victim_prod >= 2 and victim_prod in top_owned_production)
            )
            doom_ratio = 1.75 if is_core_victim else PREEMPTIVE_EVAC_DOOM_RATIO
            if threat_metric > garrison_at_deadline * doom_ratio:
