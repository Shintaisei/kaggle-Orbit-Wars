...
def _search_expand_pressure_2p(world):
    info = {
        "skip": False,
        "fleet_raw_ratio": 0.0,
        "production_diff": 0,
        ...
        "my_fleet_ships": 0,
        "my_planet_ships": 0,
        "my_prod": int(getattr(world, "my_prod", 0)),
        "opp_prod": 0,
        "my_prod_share": float(getattr(world, "my_prod_share", 0.0)),
        "owned_planets": len(getattr(world, "my_planets", [])),
    }
    if not world.is_2p or world.step < 80:
        return info
    my_planet_ships = sum(int(p.ships) for p in world.my_planets)
    my_fleet_ships = sum(int(f[6]) for f in world.fleets if int(f[1]) == world.player)
    my_raw = my_planet_ships + my_fleet_ships
    info.update({
        "my_raw": my_raw,
    ...
    enemy_owners = {
        int(owner)
        for owner in world.owner_production
        if int(owner) != world.player and int(owner) != -1
    }
    opp_prod = max((int(world.owner_production.get(owner, 0)) for owner in enemy_owners), default=0)
    for p in world.planets:
        if p.owner != world.player and p.owner != -1:
            opp_planet_ships += int(p.ships)
    for f in world.fleets:
        owner = int(f[1])
        if owner != world.player and owner != -1:
            opp_fleet_ships += int(f[6])
    fleet_raw_ratio = my_fleet_ships / my_raw
    production_diff = int(world.my_prod) - int(opp_prod)
    opp_raw = opp_planet_ships + opp_fleet_ships
    raw_diff = my_raw - opp_raw
    info.update({
        "skip": fleet_raw_ratio >= 0.65 and production_diff <= 0 and raw_diff <= 0,
        "fleet_raw_ratio": fleet_raw_ratio,
...
def _search_skip_action_digest(world, actions, limit=3):
    digest = []
    for act in actions[:limit]:
        src = world.planet_by_id.get(int(act["source_id"]))
        tgt = world.planet_by_id.get(int(act["target_id"]))
        digest.append({
            "source_id": int(act["source_id"]),
            "source_prod": None if src is None else int(src.production),
            "source_ships": None if src is None else int(src.ships),
            "target_id": int(act["target_id"]),
            "target_owner": None if tgt is None else int(tgt.owner),
            "target_prod": None if tgt is None else int(tgt.production),
            "target_ships": None if tgt is None else int(tgt.ships),
            "ships": int(act["ships"]),
            "arrival_turn": int(act["arrival_turn"]),
            "score": float(act.get("score", 0.0)),
        })
    return digest


def _trace_search_skip(world, info, actions):
    trace_path = os.environ.get("ORBIT_SEARCH_SKIP_TRACE")
    if not trace_path:
        return
    try:
        row = {
            ...
            **info,
            "top_actions": _search_skip_action_digest(world, actions),
        }
...
def handle_expand(world, available, spent, target_locked, moves, mode_log):
    ...
    search_pressure = _search_expand_pressure_2p(world)
    if SEARCH_EXPAND_4P_ENABLED and not world.is_2p:
        _handle_search_expand_4p(world, available, spent, target_locked, moves, mode_log)
    elif SEARCH_EXPAND_2P_ENABLED and world.is_2p:
        if search_pressure["skip"]:
            actions = search_step_action(
                world, max_per_source=SEARCH_MAX_PER_SOURCE,
                max_actions_to_eval=12,
                use_depth2=SEARCH_DEPTH2_ENABLED,
            )
            _trace_search_skip(world, search_pressure, actions)
        else:
            _handle_search_expand_4p(world, available, spent, target_locked, moves, mode_log)
