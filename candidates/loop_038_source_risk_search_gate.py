...
def _handle_search_expand_4p(world, available, spent, target_locked, moves, mode_log):
    ...
    for act in actions[:SEARCH_MAX_ACTIONS_TO_PICK * 2]:
        ...
        if _skip_search_action_source_risk_2p(world, src_id, tgt, act, available, spent):
            continue
...
def _search_bad_pressure_2p(world):
    ...
    raw_diff = my_raw - (opp_planet_ships + opp_fleet_ships)
    return fleet_raw_ratio >= 0.65 and production_diff <= 0 and raw_diff <= 0


def _source_has_near_hostile_arrival(world, src_id, horizon=25):
    for eta, owner, _ships in world.arrivals_by_planet.get(src_id, []):
        if owner == world.player or owner == -1:
            continue
        if int(eta) <= horizon:
            return True
...
def _skip_search_action_source_risk_2p(world, src_id, tgt, act, available, spent):
    if not _search_bad_pressure_2p(world):
        return False
    src = world.planet_by_id.get(int(src_id))
    if src is None or int(src.production) < 3:
        return False
    if tgt is None or int(tgt.owner) == -1:
        return False
    avail = int(available[src_id] - spent[src_id])
    residual = avail - int(act["ships"])
    reserve_floor = max(8, int(src.production) * 5)
    return _source_has_near_hostile_arrival(world, src_id) or residual < reserve_floor
