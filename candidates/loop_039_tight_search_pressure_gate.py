...
def _skip_search_expand_tight_pressure_2p(world):
    ...
    opp_planet_ships = sum(
        int(p.ships) for p in world.planets
        if p.owner != world.player and p.owner != -1
    )
    opp_fleet_ships = sum(
        int(f[6]) for f in world.fleets
        if int(f[1]) != world.player and int(f[1]) != -1
    )
    ...
    return fleet_raw_ratio >= 0.65 and production_diff <= -5 and raw_diff <= -500
...
def handle_expand(world, available, spent, target_locked, moves, mode_log):
    ...
    if (SEARCH_EXPAND_4P_ENABLED and not world.is_2p) or \
       (SEARCH_EXPAND_2P_ENABLED and world.is_2p and not _skip_search_expand_tight_pressure_2p(world)):
