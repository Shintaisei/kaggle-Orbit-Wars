...
def _search_first_target_clear(world, src_id, tgt_id, angle, turns, ships):
    src = world.planet_by_id.get(int(src_id))
    tgt = world.planet_by_id.get(int(tgt_id))
    if src is None or tgt is None or tgt.owner == world.player:
        return True
    lx, ly = launch_point(float(src.x), float(src.y), float(src.radius), float(angle))
    first, first_turns = fleet_target_planet(
        Fleet(-1, world.player, lx, ly, float(angle), int(src_id), int(ships)),
        world.planets,
        world.initial_by_id,
        world.ang_vel,
    )
    if first is None or first.id in (int(src_id), int(tgt_id)) or first_turns is None:
        return True
    return int(turns) - int(first_turns) < 2
    ...
    for act in actions[:SEARCH_MAX_ACTIONS_TO_PICK * 2]:
        ...
        if tgt is not None and tgt.owner == -1:
            ...
            if not _neutral_tempo_ok(world, tgt, ships_act, turns_act):
                continue
        if not _search_first_target_clear(
            world, src_id, tgt_id, act["angle"], act["arrival_turn"], act["ships"]
        ):
            continue
        if not _commit_fleet(world, moves, spent, target_locked,
