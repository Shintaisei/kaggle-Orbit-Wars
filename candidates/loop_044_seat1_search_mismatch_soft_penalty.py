...
def search_step_action(world, max_per_source=3, max_actions_to_eval=10,
    ...
    for act in actions[:max_actions_to_eval]:
        ...
        if gain > 0:
            gain *= _search_first_target_score_factor(world, act)
        act["score"] = gain
        scored.append(act)
...
def _search_first_target_score_factor(world, act):
    if not world.is_2p or world.player != 1:
        return 1.0
    ...
    if src is None or tgt is None or tgt.owner == world.player:
        return 1.0
    lx, ly = launch_point(float(src.x), float(src.y), float(src.radius), float(act["angle"]))
    first, first_turns = fleet_target_planet(
        Fleet(-1, world.player, lx, ly, float(act["angle"]), int(src.id), int(act["ships"])),
        world.planets,
        world.initial_by_id,
        world.ang_vel,
    )
    if (
        first is None
        or first.id in (int(src.id), int(tgt.id))
        or first_turns is None
        or int(act["arrival_turn"]) - int(first_turns) < 2
    ):
        return 1.0
    if first.owner == world.player:
        return 0.65
    if first.owner == -1:
        return 0.80
    return 1.0
