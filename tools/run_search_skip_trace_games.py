...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Orbit Wars games and annotate ORBIT_SEARCH_SKIP_TRACE rows.")
    ...
    previous_trace = os.environ.get("ORBIT_SEARCH_SKIP_TRACE")
    try:
        with trace_out.open("w", encoding="utf-8") as trace_fh, summary_out.open("w", encoding="utf-8") as summary_fh:
            for seed, tested_seat in args.cases:
                with tempfile.TemporaryDirectory() as tmpdir:
                    tmp_trace = Path(tmpdir) / "search_skip_trace.jsonl"
                    os.environ["ORBIT_SEARCH_SKIP_TRACE"] = str(tmp_trace)
                    summary, _turns = run_game(seed, args.agent, opponents, args.seats, tested_seat)
                ...
                print(
                    ...
                    f"skip_rows={len(trace_rows)} elapsed={summary['elapsed_s']:.3f}s"
                )
    finally:
        if previous_trace is None:
            os.environ.pop("ORBIT_SEARCH_SKIP_TRACE", None)
        else:
            os.environ["ORBIT_SEARCH_SKIP_TRACE"] = previous_trace
