...
# Neural gate v1:
# Inference-only, pure-Python MLP heads. The network does not generate raw
# angles; it only nudges safe, physics-checked missions from the 908 lineage.
NEURAL_GATE_ENABLED = True
NEURAL_TARGET_RERANK_ENABLED = False
NEURAL_TARGET_BIAS_WEIGHT_2P = 4.5
NEURAL_TARGET_BIAS_WEIGHT_4P = 2.0
_BASE_ABSORB_MIN_THREAT = ABSORB_MIN_THREAT
_BASE_DEFENSE_OVERSEND_2P = DEFENSE_OVERSEND_2P
_BASE_DEFENSE_COALITION_MAX = DEFENSE_COALITION_MAX
_BASE_HAMMER_STOCKPILE_MIN = 50
_BASE_PREEMPTIVE_EVAC_DOOM_RATIO = 1.20
_NN_LAST_GATE = 0.0
_NN_GAME_GATE = None
...
def _nn_world_features(world):
    owners = [o for o in world.owner_strength.keys() if o != -1]
    n_players = max(2, len(set(owners)))
    my_ships = float(world.owner_strength.get(world.player, 0))
    all_ships = float(sum(max(0, v) for k, v in world.owner_strength.items() if k != -1)) + 1.0
    enemy_ships = max(1.0, all_ships - my_ships)
    enemy_fleet = float(sum(int(f.ships) for f in world.fleets if f.owner not in (-1, world.player)))
    enemy_total = max(1.0, enemy_fleet + sum(int(p.ships) for p in world.enemy_planets))
    incoming = 0.0
    for p in world.my_planets:
        for _eta, owner, ships in world.arrivals_by_planet.get(p.id, []):
            if owner not in (-1, world.player):
                incoming += float(ships)
    my_planets = max(1, len(world.my_planets))
    close_enemy = 0.0
    for mp in world.my_planets:
        for ep in world.enemy_planets:
            d = max(1.0, dist(mp.x, mp.y, ep.x, ep.y))
            close_enemy += 1.0 / d
    high_prod_neutral = sum(1 for p in world.neutral_planets if int(p.production) >= 3)
    avg_garrison = my_ships / float(my_planets)
    return [
        _clip01(float(world.step) / float(TOTAL_STEPS)),
        1.0 if world.is_2p else 0.0,
        _clip01(world.my_prod_share),
        _clip01(my_ships / all_ships),
        _clip01(my_ships / enemy_ships / 2.0),
        _clip01(float(my_planets) / max(1.0, float(len(world.planets)))),
        _clip01(enemy_fleet / enemy_total),
        _clip01(incoming / max(1.0, my_ships)),
        _clip01(avg_garrison / 80.0),
        _clip01(float(len(world.neutral_planets)) / max(1.0, float(len(world.planets)))),
        _clip01(float(high_prod_neutral) / 8.0),
        _clip01(close_enemy * 12.0 / float(my_planets)),
    ]


def _nn_runtime_aggression(world):
    if not NEURAL_GATE_ENABLED:
        return 0.0
    x = _nn_world_features(world)
    hidden = []
    weights = [
        [1.2, 1.1, -0.4, 0.7, 0.8, 0.2, -1.0, -1.1, 0.9, 0.7, 0.8, -0.7],
        [-0.6, 0.8, 1.2, 1.0, 0.4, 0.5, -0.6, -1.5, 0.7, -0.2, -0.1, -0.8],
        [0.4, -0.2, -0.8, -0.5, -0.6, -0.3, 1.1, 1.5, -0.4, 0.3, 0.2, 0.9],
        [0.8, 0.4, 0.1, 0.5, 0.3, -0.2, -0.5, -0.4, 1.2, -0.6, -0.4, 0.1],
        [-0.3, 1.0, 0.6, 0.2, 0.1, 0.0, -0.4, -0.7, 0.5, 0.5, 0.8, -0.5],
        [0.7, -0.7, 0.5, 0.3, 0.3, 0.1, 0.6, 0.4, -0.2, -0.4, -0.1, 0.6],
        [-0.2, 0.5, -0.1, 0.2, 0.6, 0.4, -0.3, -0.6, 1.0, 0.3, 0.2, -0.2],
        [0.2, 0.2, 0.9, 0.7, 0.4, 0.3, -0.8, -1.0, 0.6, -0.1, 0.4, -0.6],
    ]
    biases = [-0.75, -0.35, -0.20, -0.55, -0.25, -0.45, -0.50, -0.30]
    for row, bias in zip(weights, biases):
        hidden.append(_nn_relu(sum(w * v for w, v in zip(row, x)) + bias))
    out_w = [1.05, 0.85, -0.95, 0.70, 0.80, -0.45, 0.55, 0.90]
    logit = -0.85 + sum(w * h for w, h in zip(out_w, hidden))
    if world.is_opening:
        logit += 0.25
    if not world.is_2p:
        logit += 1.45 if int(world.player) in (0, 2) else -0.20
    return _sigmoid(logit)


def _apply_neural_runtime_policy(world):
    global ABSORB_MIN_THREAT, DEFENSE_OVERSEND, DEFENSE_OVERSEND_2P, DEFENSE_COALITION_MAX
    global HAMMER_STOCKPILE_MIN, PREEMPTIVE_EVAC_DOOM_RATIO, _NN_LAST_GATE, _NN_GAME_GATE
    if _NN_GAME_GATE is None:
        _NN_GAME_GATE = _nn_runtime_aggression(world)
    gate = _NN_GAME_GATE
    _NN_LAST_GATE = gate
    aggressive = gate >= (0.25 if world.is_2p else 0.62)
    if aggressive:
        ABSORB_MIN_THREAT = 5
        DEFENSE_OVERSEND_2P = 0
        DEFENSE_COALITION_MAX = 1
        HAMMER_STOCKPILE_MIN = 45
        PREEMPTIVE_EVAC_DOOM_RATIO = 1.10
    else:
        ABSORB_MIN_THREAT = _BASE_ABSORB_MIN_THREAT
        DEFENSE_OVERSEND_2P = _BASE_DEFENSE_OVERSEND_2P
        DEFENSE_COALITION_MAX = _BASE_DEFENSE_COALITION_MAX
        HAMMER_STOCKPILE_MIN = _BASE_HAMMER_STOCKPILE_MIN
        PREEMPTIVE_EVAC_DOOM_RATIO = _BASE_PREEMPTIVE_EVAC_DOOM_RATIO
    DEFENSE_OVERSEND = DEFENSE_OVERSEND_2P if world.is_2p else DEFENSE_OVERSEND_4P
    return gate


def _reset_neural_runtime_policy():
    global ABSORB_MIN_THREAT, DEFENSE_OVERSEND, DEFENSE_OVERSEND_2P, DEFENSE_COALITION_MAX
    global HAMMER_STOCKPILE_MIN, PREEMPTIVE_EVAC_DOOM_RATIO, _NN_LAST_GATE, _NN_GAME_GATE
    ABSORB_MIN_THREAT = _BASE_ABSORB_MIN_THREAT
    DEFENSE_OVERSEND_2P = _BASE_DEFENSE_OVERSEND_2P
...
def _nn_target_delta(world, src, target, raw_dist):
    if not NEURAL_TARGET_RERANK_ENABLED:
        return 0.0
    if target.owner == world.player:
        return 0.0
    ships = max(1.0, float(int(target.ships)))
    prod = max(0.0, float(int(target.production)))
    src_ships = max(1.0, float(int(src.ships)))
    need = float(needed_to_capture(target, max(1, int(math.ceil(raw_dist / MAX_SPEED)))))
    enemy_owned = 1.0 if target.owner not in (-1, world.player) else 0.0
    neutral = 1.0 if target.owner == -1 else 0.0
    static = 1.0 if is_static_planet(target) else 0.0
    close = _clip01(1.0 - raw_dist / 80.0)
    feasible = _clip01(src_ships / max(1.0, need * 1.7))
    prod_ratio = _clip01(prod / 5.0)
    ship_tax = _clip01(ships / 80.0)
    gate = _NN_LAST_GATE
    x = [prod_ratio, feasible, close, enemy_owned, neutral, static, ship_tax, gate]
    h0 = _nn_relu(0.9*x[0] + 0.8*x[1] + 0.5*x[2] + 0.4*x[3] + 0.2*x[4] + 0.4*x[5] - 0.7*x[6] + 0.3*x[7] - 0.55)
    h1 = _nn_relu(0.4*x[0] + 1.1*x[1] + 0.9*x[2] - 0.3*x[3] + 0.5*x[4] + 0.2*x[5] - 1.0*x[6] + 0.5*x[7] - 0.45)
    h2 = _nn_relu(0.8*x[0] - 0.2*x[1] - 0.4*x[2] + 1.0*x[3] - 0.1*x[4] + 0.2*x[5] - 0.3*x[6] + 0.7*x[7] - 0.35)
    h3 = _nn_relu(-0.2*x[0] + 0.5*x[1] + 0.3*x[2] - 0.7*x[3] + 0.5*x[4] + 0.9*x[5] - 0.5*x[6] + 0.2*x[7] - 0.25)
    return math.tanh(0.75*h0 + 0.85*h1 + 0.65*h2 + 0.45*h3 - 0.45)
...
def _commit_fleet(world, moves, spent, target_locked,
    ...
    source = world.planet_by_id.get(int(src_id))
    max_allowed = int(source.ships) - int(spent[src_id]) if source is not None else int(ships)
    send = min(int(ships), max_allowed)
    if send < 1:
        return False
    moves.append([src_id, float(angle), int(send)])
    spent[src_id] += int(send)
    ...
    if os.environ.get("ORBIT_TRACE"):
        try:
            with open(os.environ["ORBIT_TRACE"], "a") as fh:
                fh.write(
                    f"t={world.step} src={src_id} tgt={target_id} ships={send} eta={turns}\n"
                )
        except Exception:
            pass
    return True
...
def _nearest_targets(src, world, K, max_travel, target_locked):
    ...
    for t in world.planets:
        ...
        if NEURAL_TARGET_RERANK_ENABLED:
            nn_weight = NEURAL_TARGET_BIAS_WEIGHT_2P if world.is_2p else NEURAL_TARGET_BIAS_WEIGHT_4P
            weighted -= nn_weight * _nn_target_delta(world, src, t, raw)
...
def plan_moves(world, deadline=None):
    ...
    _apply_neural_runtime_policy(world)
