...
# rahulchauhan016/orbit-wars-target-score-2000-4 ("Orbit Wars | Target score: 2000.4").
# Generated from the notebook Submission Export section without running benchmark/plot cells.
...
class CounterfactualRisk:
    ...
    def regret(self, action) -> float:
        ...
        P1, F1 = clone(self.s)
        sp = self.s.get(fi); dp = self.s.get(ti)
        if fi is not None and sp and dp and P1.get(fi) and P1[fi].ships >= sh > 0:
            P1[fi].ships -= sh
    ...
    def filter(self, actions: list, threshold: float = 50.0) -> list:
        # Keep actions whose positive regret stays below the risk threshold.
        result = []
        for a in actions:
            fi, ti, sh = a
            if fi is None:
